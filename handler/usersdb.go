...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	query := fmt.Sprintf("INSERT INTO `pharmacy_sh`.`users` (`name`, `surname`, `familyname`, `password`,  `age`, `address`, `phone`, `gender`, `mail, `id_pharmacy`, `zip`, `province`, `city`)  VALUES('%s', '%s', '%s', '%s', '%d', '%s', '%d', '%s', '%s', '%d', '%s', '%s', '%s')", user.Name, user.SurName, user.FamilyName, user.Password, user.Age, user.Address, user.Phone, user.Gender, user.Mail, user.IDPharmacy, user.Zip, user.Province, user.City)
