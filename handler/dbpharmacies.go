...
func GetPharmacies(w http.ResponseWriter, r *http.Request) {
	...
	if name != "" {
		FilterPharmaciesByName(name, w, r)
	} else {
		GetAllPharmacies(w, r)
	}
}
...
func FilterPharmaciesByName(name string, w http.ResponseWriter, r *http.Request) {
	pageNumber := r.URL.Query().Get("page")
	intPage, err := strconv.Atoi(pageNumber)
	elementsPage := intPage * 10
	elem := strconv.Itoa(elementsPage)
	...
	rows, err := db.DB.Table("pharmacy").Select("id, cif, address, phone_number, schedule, name, guard, mail").Where("name LIKE ?", fmt.Sprintf("%%%s%%", name)).Offset(elem).Limit(10).Rows()
